...
const AutoTest = ({ chapterIds, formKey, subjectId }) => {
    ...
    testRandomService(
      {
        ...
        generateConfig: {
          numEasyQuestion: Number(value.generateConfig.numEasyQuestion),
          numMediumQuestion: Number(value.generateConfig.numMediumQuestion),
          numHardQuestion: Number(value.generateConfig.numHardQuestion),
  ...
  const checkConfigTotal = (rule, value) => {
    const total = Number(easyNumber) + Number(mediumNumber) + Number(hardNumber);
		console.log(total);
  ...
  return (
    <div className="test-create-view">
      <Form onFinish={onFinish} name="test-create" key={formKey} form={form}>
      <Form.Item
          name="name"
          label="Tên kỳ thi:"
          rules={[
        ...
        >
          <DatePicker format={"YYYY-MM-DD HH:mm"} showTime={{format: "HH:mm"}}></DatePicker>
        </Form.Item>
        <Form.Item
          name="endTime"
          label="Thời gian kết đầu:"
          rules={[
            {
              required: true,
              message: "Chưa chọn ngày",
        ...
        >
          <Row gutter={5}>
            <Col span={15}>
              <Form.Item name={["generateConfig", "numEasyQuestion"]}>
                <Input placeholder="Dễ" type="number" onChange={e => setEasyNumber(e.target.value)}/>
              </Form.Item>
            </Col>
            <Col span={15}>
              <Form.Item name={["generateConfig", "numMediumQuestion"]}>
                <Input placeholder="Trung bình" type="number" onChange={e => setMediumNumber(e.target.value)}/>
              </Form.Item>
            </Col>
            <Col span={15}>
              <Form.Item name={["generateConfig", "numHardQuestion"]}>
                <Input placeholder="Khó" type="number" onChange={e => setHardNumber(e.target.value)}/>
              </Form.Item>
            </Col>
          </Row>
        </Form.Item>
